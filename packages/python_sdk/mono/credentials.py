...
DEFAULT_DEV_API_TOKEN = "rt_mono_dev"


def credentials_path() -> Path:
    override = os.getenv("MONO_CREDENTIALS_FILE")
    if override:
        return Path(override).expanduser()
    config_home = Path(os.getenv("XDG_CONFIG_HOME", Path.home() / ".config"))
    return config_home / "mono" / "credentials.json"
...
def resolve_connection(
    ...
    resolved_api_token = (
        api_token
        or os.getenv("MONO_API_TOKEN")
        or os.getenv("MONO_API_KEY")
        or saved_api_token
