...
function AddTokenDialog({ projects, onCreated }: { projects: Project[]; onCreated: (token: ApiToken) => void }) {
  ...
  const reset = () => { sessionRef.current += 1; setName("Agent CLI"); setExpires("90"); setSecret(null); setCopied(false); setProjectIds(null); setBusy(requestInFlightRef.current) }
  const close = () => { setOpen(false); reset() }
  const createToken = async () => {
    const session = sessionRef.current
    if (requestInFlightRef.current || createdSessionRef.current === session) return

    requestInFlightRef.current = true
    setBusy(true)
    try {
      const result = await auth.createToken({ name, expires_in_days: expires ? Number(expires) : null, project_ids: selectedProjectIds })
      createdSessionRef.current = session
      onCreated(result.api_token)
      if (sessionRef.current === session) {
        setSecret(result.token)
        setCopied(false)
      }
    } catch (error) {
      if (sessionRef.current === session) toast.error(error instanceof Error ? error.message : "Could not create token")
    } finally {
