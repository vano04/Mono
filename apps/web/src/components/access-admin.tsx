...
function AddTokenDialog({ projects, onCreated }: { projects: Project[]; onCreated: (token: ApiToken) => void }) {
  ...
  const reset = () => { sessionRef.current += 1; submitIntentRef.current = false; setName("Agent CLI"); setExpires("90"); setSecret(null); setCopied(false); setProjectIds(null); setBusy(requestInFlightRef.current) }
