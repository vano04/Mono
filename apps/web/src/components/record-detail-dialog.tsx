...
function ImplementationDetails({ value }: { value: string }) {
  ...
  return <div className="min-w-0 space-y-3">
    ...
    <div className="max-w-full overflow-hidden rounded-lg border bg-zinc-950 text-zinc-100"><div className="flex items-center gap-2 border-b border-white/10 px-4 py-2 text-xs text-zinc-400"><GitPullRequest className="size-3.5" />Proposed code diff</div><pre className="max-w-full overflow-x-auto p-4 font-mono text-xs leading-5">{diff.trim().split("\n").map((line, index) => <span key={`${index}-${line}`} className={`block ${line.startsWith("+") && !line.startsWith("+++") ? "bg-emerald-500/15 text-emerald-200" : line.startsWith("-") && !line.startsWith("---") ? "bg-red-500/15 text-red-200" : line.startsWith("@@") ? "text-sky-300" : ""}`}>{line}</span>)}</pre></div>
...
function ExperimentDetails({ experiment }: { experiment: Experiment }) {
  return <div className="flex min-w-0 flex-col gap-5 p-4 sm:gap-7 sm:p-7">
    <DialogHeader className="pr-10">
      ...
      <DialogTitle className="text-lg leading-tight sm:text-xl">{experiment.title}</DialogTitle>
      <DialogDescription className="break-words">{experiment.hypothesis || "No hypothesis recorded."}</DialogDescription>
    </DialogHeader>
    <dl className="grid min-w-0 grid-cols-2 gap-px overflow-hidden rounded-lg border bg-border lg:grid-cols-4">
      <div className="min-w-0 bg-background p-3 sm:p-4"><dt className="text-xs text-muted-foreground">Source</dt><dd className="mt-1 truncate text-sm">{experiment.source || "—"}</dd></div>
      <div className="min-w-0 bg-background p-3 sm:p-4"><dt className="text-xs text-muted-foreground">Model</dt><dd className="mt-1 truncate text-sm">{experiment.source_model || "—"}</dd></div>
      <div className="min-w-0 bg-background p-3 sm:p-4"><dt className="text-xs text-muted-foreground">Implementation branch</dt><dd className="mt-1 truncate font-mono text-xs">{experiment.claimed_by || "Not claimed"}</dd></div>
      <div className="min-w-0 bg-background p-3 sm:p-4"><dt className="text-xs text-muted-foreground">Metric mode</dt><dd className="mt-1 font-mono text-sm">{experiment.metric_mode}</dd></div>
    </dl>
    {experiment.reasoning ? <section className="min-w-0"><h3 className="mb-2 text-sm font-medium">Reasoning</h3><p className="break-words text-sm leading-6 text-muted-foreground">{experiment.reasoning}</p></section> : null}
    {experiment.implementation_details ? <section className="min-w-0"><h3 className="mb-2 text-sm font-medium">Implementation details</h3><ImplementationDetails value={experiment.implementation_details} /></section> : null}
    <section className="min-w-0"><h3 className="mb-3 flex items-center gap-2 text-sm font-medium"><FileJson className="size-4" />Configuration</h3>{Object.keys(experiment.configuration).length ? <pre className="max-h-80 max-w-full overflow-auto whitespace-pre-wrap break-words rounded-lg bg-muted p-4 font-mono text-xs leading-5">{JSON.stringify(experiment.configuration, null, 2)}</pre> : <p className="text-sm text-muted-foreground">No configuration recorded.</p>}</section>
  </div>
}
...
function RunDetails({ run, baselineRun, metric, reload }: { run: Run; baselineRun: Run | null; metric: string; reload: () => void }) {
  ...
  return <div className="flex min-w-0 flex-col gap-5 p-4 sm:gap-7 sm:p-7">
    <DialogHeader className="pr-10">
      ...
      <DialogTitle className="text-lg leading-tight sm:text-xl">{run.name}</DialogTitle>
      <DialogDescription className="break-words">{run.hypothesis || "No hypothesis recorded."}</DialogDescription>
    </DialogHeader>
    <dl className="grid min-w-0 grid-cols-2 gap-px overflow-hidden rounded-lg border bg-border sm:grid-cols-3">
      <div className="col-span-2 bg-background p-3 sm:col-span-1 sm:p-4"><dt className="text-xs text-muted-foreground">Started</dt><dd className="mt-1 font-mono text-xs">{new Date(run.started_at).toLocaleString()}</dd></div>
      <div className="min-w-0 bg-background p-3 sm:p-4"><dt className="text-xs text-muted-foreground">Run branch</dt><dd className="mt-1 truncate font-mono text-xs">{run.git_branch || "—"}</dd></div>
      <div className="min-w-0 bg-background p-3 sm:p-4"><dt className="text-xs text-muted-foreground">Commit</dt><dd className="mt-1 truncate font-mono text-xs">{run.git_commit?.slice(0, 10) || "—"}</dd></div>
    </dl>
    {run.reasoning ? <section className="min-w-0"><h3 className="mb-2 text-sm font-medium">Reasoning</h3><p className="break-words text-sm leading-6 text-muted-foreground">{run.reasoning}</p></section> : null}
    {run.change_summary ? <section className="min-w-0"><h3 className="mb-2 text-sm font-medium">What changed</h3><p className="break-words text-sm leading-6 text-muted-foreground">{run.change_summary}</p></section> : null}
    {run.metric_mode === "curve" && curveMetric && run.metrics?.[curveMetric]?.points.length ? <section className="min-w-0 rounded-xl border bg-background/60 p-3 sm:p-5"><div className="mb-3"><h3 className="text-sm font-medium">{curveMetric} vs baseline</h3><p className="mt-1 text-xs text-muted-foreground">{run.lifecycle === "running" ? "New points appear here live as they are logged." : `Every recorded ${curveMetric} point across the shared step range.`}</p></div><RunCurveChart run={run} baseline={baselineRun} metric={curveMetric} /></section> : null}
    ...
    {metricEntries.length ? <section className="min-w-0"><h3 className="mb-3 text-sm font-medium">{run.metric_mode === "timings" ? "Timing results" : "Metrics"}</h3><div className="grid gap-3 sm:grid-cols-2">{metricEntries.map(([name, summary]) => <div key={name} className="rounded-lg border p-4 sm:p-5"><span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{name.replaceAll("_", " ")}</span><strong className={`mt-2 block font-mono ${run.metric_mode === "timings" || run.metric_mode === "scalar" ? "text-4xl tracking-tight" : "text-2xl"}`}>{summary.latest}</strong><small className="mt-2 block text-muted-foreground">{summary.count} {summary.count === 1 ? "point" : "points"} · range {summary.min}–{summary.max}</small></div>)}</div></section> : null}
    {run.result_summary || run.conclusion ? <section className="min-w-0 rounded-lg border bg-muted/40 p-4 sm:p-5"><h3 className="text-sm font-medium">Conclusion</h3><p className="mt-2 break-words text-sm leading-6 text-muted-foreground">{run.conclusion || run.result_summary}</p></section> : null}
