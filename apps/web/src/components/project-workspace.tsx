...
function DashboardView({ data, progress, slug, canEdit, reload, setProgress, openRecord, onProgressQueryChange }: {
  data: Dashboard; progress: ProgressData; slug: string; canEdit: boolean; reload: () => void; setProgress: (value: ProgressData) => void; openRecord: (selection: NonNullable<RecordSelection>) => void; onProgressQueryChange: (value: { metric: string; window: string; includeTags: string[]; excludeTags: string[] }) => void
}) {
  ...
  return <>
    <PageHeading title="Dashboard" description={data.project.description || (canEdit ? "Add a durable research goal in Settings so every agent starts from the same objective." : "No project goal has been recorded.")} actions={canEdit ? <CreateExperimentDialog slug={slug} onCreated={reload} /> : undefined} />
    ...
    <Card className="mb-8 py-0">
      <CardContent className="grid p-0 sm:grid-cols-2 xl:grid-cols-[1.4fr_.65fr_.65fr_auto]">
        <div className="border-b p-5 sm:border-r xl:border-b-0"><span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Current main baseline</span>{data.baseline ? <button className="mt-2 block text-left" onClick={() => openRecord({ kind: "run", id: data.baseline!.id })}><strong className="text-base">{data.baseline.display_id} · {data.baseline.name}</strong><span className="mt-1 block text-xs text-muted-foreground">Established {formatDate(data.baseline.finished_at)}</span></button> : <p className="mt-2 text-sm text-muted-foreground">No completed baseline yet.</p>}</div>
        <div className="border-b p-5 xl:border-b-0 xl:border-r"><span className="text-xs text-muted-foreground">Primary metric</span><strong className="mt-2 block font-mono text-lg">{data.baseline ? latestMetric(data.baseline, data.project.progress_metric_key) : "—"}</strong></div>
        <div className="border-b p-5 sm:border-b-0 sm:border-r xl:border-r-0"><span className="text-xs text-muted-foreground">Recorded workers</span><strong className="mt-2 block font-mono text-lg">{data.worker_count}</strong></div>
        <div className="flex items-center px-5 py-4 xl:border-l"><Badge variant="secondary"><Database />Shared registry</Badge></div>
      </CardContent>
    </Card>
    ...
    <section className="mb-10">
      ...
      {queue.length ? <><div className="grid gap-3 md:hidden">{queue.map((item) => {
        const isRun = "change_summary" in item
        return <div key={`mobile-${isRun ? "run" : "experiment"}-${item.id}`} className="flex items-start gap-3 rounded-lg border p-4"><button type="button" className="min-w-0 flex-1 text-left" onClick={() => openRecord({ kind: isRun ? "run" : "experiment", id: item.id })}><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xs text-muted-foreground">{item.display_id}</span><StatusBadge value={item.lifecycle} /></div><strong className="mt-2 block text-sm font-medium">{isRun ? item.name : item.title}</strong><span className="mt-1 line-clamp-2 block text-xs leading-5 text-muted-foreground">{item.hypothesis}</span></button><RecordActions slug={slug} id={item.id} type={isRun ? "run" : "experiment"} canEdit={canEdit} onChanged={reload} /></div>
      })}</div><div className="hidden overflow-hidden rounded-lg border md:block"><Table><TableHeader><TableRow><TableHead className="w-28">ID</TableHead><TableHead>Status</TableHead><TableHead>Experiment</TableHead><TableHead className="hidden xl:table-cell">Branch</TableHead><TableHead className="hidden lg:table-cell">Created</TableHead><TableHead className="w-12"><span className="sr-only">Actions</span></TableHead></TableRow></TableHeader><TableBody>{queue.map((item) => {
        const isRun = "change_summary" in item
        const branch = isRun ? item.git_branch : item.claimed_by
        return <TableRow key={`${isRun ? "run" : "experiment"}-${item.id}`} className="cursor-pointer" onClick={() => openRecord({ kind: isRun ? "run" : "experiment", id: item.id })}><TableCell className="font-mono text-xs">{item.display_id}</TableCell><TableCell><StatusBadge value={item.lifecycle} /></TableCell><TableCell><strong className="block max-w-lg truncate text-sm font-medium">{isRun ? item.name : item.title}</strong><span className="mt-1 block max-w-lg truncate text-xs text-muted-foreground">{item.hypothesis}</span></TableCell><TableCell className="hidden xl:table-cell"><code className="text-xs text-muted-foreground">{branch || "Not claimed"}</code></TableCell><TableCell className="hidden text-xs text-muted-foreground lg:table-cell">{formatDate(isRun ? item.started_at : item.created_at)}</TableCell><TableCell onClick={(event) => event.stopPropagation()}><RecordActions slug={slug} id={item.id} type={isRun ? "run" : "experiment"} canEdit={canEdit} onChanged={reload} /></TableCell></TableRow>
      })}</TableBody></Table></div></> : <Empty className="min-h-56 border"><EmptyHeader><EmptyMedia variant="icon"><FlaskConical /></EmptyMedia><EmptyTitle>The queue is empty</EmptyTitle><EmptyDescription>{canEdit ? "Propose an experiment here or let a planning agent add one through MCP." : "No work is currently queued for this project."}</EmptyDescription></EmptyHeader></Empty>}
    </section>

    <section className="md:flex md:h-[calc(100dvh-2.5rem)] md:min-h-[32rem] md:flex-col">
      ...
      {data.history.length ? <><div className="grid gap-3 md:hidden">{data.history.map((run) => <div key={`mobile-${run.id}`} className="flex items-start gap-3 rounded-lg border p-4"><button type="button" className="min-w-0 flex-1 text-left" onClick={() => openRecord({ kind: "run", id: run.id })}><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xs text-muted-foreground">{run.display_id}</span><StatusBadge value={run.lifecycle === "crashed" ? "crashed" : run.disposition} /></div><strong className="mt-2 block text-sm">{run.name}</strong><span className="mt-1 block font-mono text-xs text-muted-foreground">{run.result_summary || latestMetric(run, data.project.progress_metric_key)}</span><span className="mt-2 block text-xs text-muted-foreground">Finished {formatDate(run.finished_at)}</span></button><RecordActions slug={slug} id={run.id} type="run" canEdit={canEdit} canBaseline={run.lifecycle === "completed"} onChanged={reload} /></div>)}</div><ScrollArea className="hidden min-h-0 flex-1 overflow-hidden rounded-xl ring-1 ring-foreground/10 md:block [&_[data-slot=table-container]]:overflow-visible [&>[data-slot=scroll-area-scrollbar]]:!top-10 [&>[data-slot=scroll-area-scrollbar]]:h-[calc(100%-2.5rem)]"><Table><TableHeader className="sticky top-0 z-10 bg-card"><TableRow><TableHead>Run</TableHead><TableHead className="hidden xl:table-cell">Branch</TableHead><TableHead className="hidden lg:table-cell">Finished</TableHead><TableHead>Result</TableHead><TableHead>Disposition</TableHead><TableHead className="w-12"><span className="sr-only">Actions</span></TableHead></TableRow></TableHeader><TableBody>{data.history.map((run) => <TableRow key={run.id} className="cursor-pointer" onClick={() => openRecord({ kind: "run", id: run.id })}><TableCell><span className="font-mono text-xs text-muted-foreground">{run.display_id}</span><strong className="mt-1 block max-w-sm truncate text-sm">{run.name}</strong></TableCell><TableCell className="hidden xl:table-cell"><code className="text-xs text-muted-foreground">{run.git_branch || "—"}</code></TableCell><TableCell className="hidden text-xs text-muted-foreground lg:table-cell">{formatDate(run.finished_at)}</TableCell><TableCell className="max-w-xs truncate font-mono text-xs">{run.result_summary || latestMetric(run, data.project.progress_metric_key)}</TableCell><TableCell><StatusBadge value={run.lifecycle === "crashed" ? "crashed" : run.disposition} /></TableCell><TableCell onClick={(event) => event.stopPropagation()}><RecordActions slug={slug} id={run.id} type="run" canEdit={canEdit} canBaseline={run.lifecycle === "completed"} onChanged={reload} /></TableCell></TableRow>)}</TableBody></Table></ScrollArea></> : <Empty className="min-h-48 flex-1 border"><EmptyHeader><EmptyMedia variant="icon"><Activity /></EmptyMedia><EmptyTitle>No completed runs</EmptyTitle><EmptyDescription>Tracked runs will appear here after an agent finishes or crashes them.</EmptyDescription></EmptyHeader></Empty>}
    </section>
  </>
}
...
function SearchView({ data, slug, canEdit, reload, openRecord }: { data: Dashboard; slug: string; canEdit: boolean; reload: () => void; openRecord: (selection: NonNullable<RecordSelection>) => void }) {
...
function ArchiveView({ data, slug, canEdit, reload, openRecord }: { data: Dashboard; slug: string; canEdit: boolean; reload: () => void; openRecord: (selection: NonNullable<RecordSelection>) => void }) {
  return <><PageHeading title="Archive" description="Archived records are excluded from active dashboards, claims, ordinary search, and default agent context." />
    {data.archived.length ? <div className="divide-y border-y">{data.archived.map((item) => { const isRun = "change_summary" in item; return <div key={item.id} className="grid grid-cols-[1fr_auto] items-center gap-5 py-5"><button className="text-left" onClick={() => openRecord({ kind: isRun ? "run" : "experiment", id: item.id })}><span className="font-mono text-xs text-muted-foreground">{item.display_id}</span><strong className="mt-1 block text-sm">{isRun ? item.name : item.title}</strong><p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{item.hypothesis}</p></button><RecordActions slug={slug} id={item.id} type={isRun ? "run" : "experiment"} canEdit={canEdit} archived onChanged={reload} /></div> })}</div> : <Empty className="min-h-72 border"><EmptyHeader><EmptyMedia variant="icon"><Archive /></EmptyMedia><EmptyTitle>Nothing is archived</EmptyTitle><EmptyDescription>Archived experiments and runs remain restorable and auditable.</EmptyDescription></EmptyHeader></Empty>}
  </>
}

function SettingsView({ data, slug, canManage, reload }: { data: Dashboard; slug: string; canManage: boolean; reload: () => void }) {
...
export function ProjectWorkspace({ slug, view }: { slug: string; view: ProjectView }) {
  ...
  return <ProjectShell project={data.project} accessRole={data.access_role}>
    {view === "dashboard" ? <DashboardView data={data} progress={progress} slug={slug} canEdit={canEdit} reload={load} setProgress={setProgress} openRecord={setSelectedRecord} onProgressQueryChange={(value) => { progressQuery.current = value }} /> : null}
    {view === "search" ? <SearchView data={data} slug={slug} canEdit={canEdit} reload={load} openRecord={setSelectedRecord} /> : null}
    {view === "archive" ? <ArchiveView data={data} slug={slug} canEdit={canEdit} reload={load} openRecord={setSelectedRecord} /> : null}
    {view === "settings" ? canEdit ? <SettingsView key={data.project.updated_at + data.program.version} data={data} slug={slug} canManage={canManage} reload={load} /> : <ReadOnlySettingsView /> : null}
    <RecordDetailDialog selection={selectedRecord} slug={slug} baselineId={data?.baseline?.id ?? null} metric={data?.project.progress_metric_key ?? "validation_loss"} canEdit={canEdit} onClose={closeRecord} />
  </ProjectShell>
}
