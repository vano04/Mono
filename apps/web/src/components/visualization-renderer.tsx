...
const payload=${payload};window.mono={datasets:payload.datasets,theme:payload.theme};document.getElementById("widget-styles").textContent=payload.styles;document.getElementById("widget-root").innerHTML=payload.markup;window.addEventListener("error",event=>{const root=document.getElementById("widget-root");root.innerHTML='<div class="card"><strong>Widget error</strong><p class="muted"></p></div>';root.querySelector("p").textContent=event.message;});const script=document.createElement("script");script.textContent=payload.script;document.body.appendChild(script);
</script></body></html>`
}
