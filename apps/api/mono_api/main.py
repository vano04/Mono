...
def health() -> dict[str, str]:
    return {"status": "ok", "service": "mono-api"}
...
def get_result_visualization_guide(project: str, session: Session = Depends(get_db)) -> dict:
    ...
    return {
        ...
        "rules": [
            "Use mono run_metrics datasets for recorded experiment data.",
        ...
        "dataset": {"source": "mono", "query": "run_metrics", "fields": ["name", "value", "step", "timestamp", "context fields"], "filters": ["latest_per_name", "sort_by", "order", "limit"]},
    }
...
def create_result_visualization(project: str, body: ResultVisualizationTypeCreate, session: Session = Depends(get_db)) -> dict:
    ...
    if not any(dataset.get("source") == "mono" and dataset.get("query") == "run_metrics" for dataset in document["datasets"].values()):
...
def export_visualization(project: str, visualization_id: str, session: Session = Depends(get_db)) -> dict:
    ...
    if item.source_run_id:
        ...
        for name, dataset in portable_spec.get("datasets", {}).items():
            if dataset.get("source") == "mono" and dataset.get("query") == "run_metrics":
...
def import_visualization(project: str, body: VisualizationImport, session: Session = Depends(get_db)) -> dict:
    ...
    if document.get("format") != "mono-visualization" or document.get("version") != 1 or not isinstance(document.get("visualization"), dict):
        raise HTTPException(422, "Unsupported Mono visualization document")
