...
ROOT = SOURCE_ROOT if (SOURCE_ROOT / "alembic.ini").exists() else Path(os.getenv("MONO_ROOT", Path.cwd())).resolve()


class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="MONO_", env_file=ROOT / ".env", extra="ignore")

    database_url: str = "postgresql+psycopg://mono:mono@localhost:5432/mono"
