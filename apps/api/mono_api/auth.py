...
def apply_owner_recovery_password(session: Session) -> None:
    ...
    if not password:
        return
    if len(password) < 12:
        raise RuntimeError("MONO_OWNER_RECOVERY_PASSWORD must contain at least 12 characters")
...
async def authenticate_request(request: Request, call_next):
    ...
    if path.startswith("/api/") and not request.state.identity:
        detail = "This Mono instance needs an owner" if not configured else "Sign in with a password or provide a valid bearer token"
