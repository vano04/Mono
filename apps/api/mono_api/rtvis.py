...
RTVIS_SCHEMA_URL = "https://mono.dev/schemas/rtvis/v1.json"
...
class RTVisDataset(StrictModel):
    source: Literal["inline", "mono"] = "inline"
    ...
    def validate_source(self) -> RTVisDataset:
        if self.source == "inline" and self.query is not None:
            raise ValueError("Inline datasets cannot declare a Mono query")
        if self.source == "mono" and self.query is None:
            raise ValueError("Mono datasets must declare a query")
        if self.source == "mono" and self.rows:
            raise ValueError("Mono datasets cannot include inline rows")
        return self
...
def normalized_spec(spec: RTVisSpec) -> dict[str, Any]:
    ...
    for name, dataset in spec.datasets.items():
        if dataset.source == "inline":
            ...
            datasets[name] = {"source": "mono", "query": dataset.query}
            if dataset.filters:
...
def visualization_guide(saved_visualizations: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    return {
        ...
        "rules": [
            ...
            "Use chart nodes for data marks; Mono applies project theme tokens automatically.",
            "Use a javascript node only when the trusted nodes cannot express the interaction.",
            "JavaScript widgets receive window.mono.datasets and window.mono.theme and run in an isolated, network-disabled iframe.",
        ...
        "dataset_sources": {
            ...
            "mono": {"queries": ["runs", "experiments", "run_metrics"], "description": "Live project-scoped data resolved by Mono. Dashboard run_metrics datasets require source_run_id; reusable experiment result types use their separate guide."},
        },
