...
Every execution attempt has exactly one run. Before creating it, determine whether the command already uses the SDK, is wrapped by `mono exec`, or needs MCP tracking.

- Let embedded SDK instrumentation create the run when it already owns the lifecycle.
- If MCP creates the run and the child process uses the SDK, pass the returned ID as `MONO_RUN_ID`; the SDK attaches to that running record.
