...
uploads to the returned run ID through the Mono API. Never substitute a
...
2. **Check shared state**: retrieve the Mono project context. If the shared
...
6. **Review existing evidence**: summarize the current Mono baseline, best
   relevant completed runs, failed approaches, active claims, and exclusions.
   Cite actual Mono run or experiment IDs. Never invent IDs or results.
7. **Confirm and go**: confirm that the branch, data, Mono connection, and
...
- Commit `Data/logs/` or cached datasets. Upload experiment logs and generated
  outputs to their Mono run instead. Config files under `Data/configs/` are
  runnable source and may be committed with the experiment, but every run must
  also store its exact resolved config in Mono.
- Reset or overwrite human changes unrelated to the current experiment.
- Delete, archive, retag, or otherwise rewrite unrelated Mono records.
...
optimizer quality is the main target unless the human or Mono project metric
...
create a local row. Cite and use that Mono run as the baseline. Re-run the
...
2. Start one tracked Mono run before launching training, preferably with
...
3. Save the returned run ID. All metrics and events for the process must use
   that actual ID.
4. Parse the active YAML and call `log_config` so Mono stores its resolved
...
6. Log validation checkpoints to Mono as they appear.
...
baseline run ID so the human can select it as the project baseline in Mono.
...
run is finished or aborted, store its complete contents in Mono as a
previewable text artifact named `training.log` with kind `log`. If the file is
too large for the configured Mono artifact limit, upload a compressed full
log as an artifact and a previewable text tail containing the final metrics and
error context. Do not delete the only complete copy until Mono confirms the
upload.
...
Git still stores runnable source history. Mono stores the self-contained
...
validation checkpoints. Log meaningful validation checkpoints to Mono as
they arrive.
...
- for a process crash, upload the complete log first, record the exit status and
  concise stack-trace summary, then use the SDK `abort` operation or Mono
...
For every experiment, preserve in Mono:
...
related evidence. Compare against the current Mono baseline and best
...
1. Inspect Git state and retrieve fresh Mono project context.
2. Search Mono for prior attempts related to the next mechanism or
   hyperparameter.
...
7. Start exactly one Mono run before execution, preferably through the
...
12. If the output is empty, inspect the last 80 lines of the log, record the
    error in Mono, and attempt a fix. Reuse the same run only while repairing
    ...
    on.
13. Upload the complete spool as a previewable `training.log` Mono artifact.
    ...
    run.
14. Log final metrics and finish or abort the Mono run. Use `kept` only when
...
The branch advances through kept experiments. Mono preserves both kept and
...
the evidence, finish the Mono run, and treat it as a failure.

**Crashes**: if a run crashes because of OOM, a typo, an invalid config, a
non-finite optimizer state, or another error, record the failure in Mono. If
...
search Mono again, compare the strongest and weakest completed runs,
